...
Usage:
    ...
    python init_db_v2.py --rebuild-all  # WARNING: also drops curation tables

Deleting strain_db_v2.sqlite and re-running reproduces the same database.

SAFETY: By default, manual_metadata and manual_curation_audit are PRESERVED
across rebuilds. Use --rebuild-all only if you explicitly want to destroy
...
# Tables that hold human-curated data and must survive rebuilds
CURATION_TABLES = {"manual_metadata", "manual_curation_audit", "manual_phenotypes"}
...
def main():
    ...
    parser.add_argument("--rebuild-all", action="store_true",
                        help="Also drop curation tables (manual_metadata, audit)")
    ...
    if db_path.exists():
        if not args.rebuild_all:
            # Preserve curation tables by exporting them first
            old_conn = sqlite3.connect(str(db_path))
            curation_backup = {}
            for ct in CURATION_TABLES:
                try:
                    rows = old_conn.execute(f"SELECT * FROM {ct}").fetchall()
                    cols = [d[0] for d in old_conn.execute(
                        f"PRAGMA table_info({ct})").fetchall()]
                    col_names = [c[1] for c in
                                 old_conn.execute(f"PRAGMA table_info({ct})").fetchall()]
                    if rows:
                        curation_backup[ct] = (col_names, rows)
                        print(f"  Preserving {ct}: {len(rows)} rows")
    ...
    # Restore curation tables if preserved
    if not args.rebuild_all and db_path.name:  # always True here, just for clarity
        for ct, (col_names, rows) in curation_backup.items():
            if not rows:
                continue
            placeholders = ",".join(["?"] * len(col_names))
            cols_str = ",".join(col_names)
            sql = f"INSERT OR IGNORE INTO {ct} ({cols_str}) VALUES ({placeholders})"
            for row in rows:
                conn.execute(sql, row)
            conn.commit()
            restored = conn.execute(f"SELECT COUNT(*) FROM {ct}").fetchone()[0]
            print(f"  Restored {ct}: {restored} rows")
